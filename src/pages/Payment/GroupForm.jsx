...
export default function GroupForm() {
    ...
    const [group, setGroup] = useState({
        ...
        maxMembers: 3,
        members: [
            { id: 1, name: "John Doe", nim: "2802312312" },
            { id: 2, name: "Jane Smith", nim: "2802312456" },
            { id: 3, name: "Jane Smith", nim: "2802312456" },
        ],
    });
    ...
    const handleConfirm = () => {};
    const handleBack = () => { setPopupOpen(true); };
    const handleDeleteAllMembers = () => { setDeleteAllPopupOpen(true); };
                    ...
                    <div className="flex flex-col items-center gap-5">
                            ...
                            <div className="flex w-full">
                                <Card className="flex flex-col justify-between p-10 rounded-xl border-white border-[3px] w-full gap-13">
                                    <div className="flex flex-col gap-3">
                                        <h1 className="text-4xl font-bold text-primary">Group</h1>
                                        ...
                                        <div>
                                            <p className="text-xs sm:text-lg">
                                                Number of code users ({group.members.filter((m) => m.name.trim() !== "").length}/{group.maxMembers}):
                                            </p>
                                                ...
                                                {group.members.map((member, index) => (
                                                    <div key={member.id} className="flex items-center gap-2">
                                                        ...
                                                        <div className="flex-1">
                                                            {member.name ? `${member.name} - ${member.nim}` : ""}
                                                        </div>
                                                        {isEditing && (
                                                            <button
                                                                className="flex-shrink-0"
                                                                onClick={() => {
                                                                    const updated = group.members.filter((_, memberIndex) => memberIndex !== index);
                                                                    updated.push({ id: Date.now(), name: "", nim: "" });
                                                                    setGroup({ ...group, members: updated });
                                                                }}
                                                            >
                                                                <img src={IconTrash} alt="Delete" className="w-[18px] h-[18px] cursor-pointer" />
                                                            </button>
                                                        )}
                                                    </div>
                                                ))}
                                ...
                                <Card className="flex flex-col p-10 rounded-xl border-white border-[3px] w-full">
                                    <div className="flex flex-col gap-3">
                                        <div className="flex items-center justify-between">
                                            <h1 className="text-xl font-bold sm:text-3xl">Payment Submission</h1>
                                            {/* Kirim status atau isPending ke PaymentStatusButton */}
                                            <PaymentStatusButton status={paymentStatus} isPaid={isPaid} />
                                        </div>

                                        <div>
                                            <p className="text-xs sm:text-lg">Registration Mode</p>
                                            <p className="mt-1 text-sm sm:text-lg font-semibold">{DUMMY_REGISTRATION_MODE}</p>
                                        </div>

                                        <div>
                                            <p className="text-xs sm:text-lg">Package Type</p>
                                            <p className="mt-1 text-sm sm:text-lg font-semibold">{DUMMY_PACKAGE_TYPE}</p>
                                        </div>

                                        <div>
                                            <p className="text-xs sm:text-lg">Fee</p>
                                            <p className="mt-1 text-sm sm:text-lg font-semibold">{DUMMY_FEE}</p>
                                        </div>
                                    </div>
                                    <div className="flex flex-col gap-3 mt-5 items-center">
                                        <div className="w-fit">
                                            {/* Disabled jika sudah bayar, pending, atau anggota belum penuh */}
                                            <Button disabled={isPaid || isPending || !isMembersFull} onClick={handlePayment}>
                                                PAY NOW
                                            </Button>
                                        </div>
                                        {!isMembersFull && !isPending && !isPaid && (
                                            <p className="text-sm text-center text-[#0A2745]/70">
                                                Button will be active after group has 3 members.
                                            </p>
                                        )}
                                        {isPending && (
                                            <p className="text-center text-sm text-[#0A2745]/70">
                        ...
                        <div className="w-full">
                            <div className="relative mt-2 flex items-center justify-start">
                                <div className="w-fit rounded-full border border-white bg-white/40 py-3 pl-20 pr-6 text-sm sm:text-lg text-[#0A2745]">
                                    Looking for teammates? Join our WhatsApp group to find others!
                                </div>
                                <div className="absolute left-0 w-16 h-16 rounded-full bg-white border-[6px] border-white flex items-center justify-center shadow-md">
                        ...
                        <div className="flex flex-col items-center gap-3">
                            <div className="flex gap-4">
                                ...
                                <Button onClick={handleConfirm} disabled={!isPaid}>
                                    REGISTRATION
                                </Button>
                            </div>
                            {!isPaid && (
                                <p className="text-sm text-center text-primary">
                                    {isPending 
                                        ? "Registration button will be active after payment verification is complete." 
