...
export default function GuidelineSection({
  ...
  const handleToggle = () => {
    const newValue = !hasReadGuideline
    setHasReadGuideline(newValue)
  ...
  return (
    <Card className="flex flex-col rounded-xl border-white border-[3px] gap-4 px-7 py-6 sm:px-14 sm:py-10 xl:px-16 xl:py-12 sm:gap-6">
      ...
      >
        ...
        />
        <div
          className={`flex items-center justify-center size-5 sm:size-6 transition-colors border-2 rounded-sm shrink-0 ${
            hasReadGuideline
              ? 'bg-persian-indigo border-persian-indigo'
              : 'bg-transparent border-[#2474C0]'
          }`}
        >
          <Check
            className={`size-3.5 sm:size-5 text-white transition-opacity ${
              hasReadGuideline ? 'opacity-100' : 'opacity-0'
            }`}
            strokeWidth={3}
            aria-hidden="true"
          />
