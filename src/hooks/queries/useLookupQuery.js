...
export default function useLookupQuery(regionId, facultyId) {
  ...
  const extractData = (resData, fallback, isRegion = false) => {
    let list = fallback;
    ...
    if (isRegion) {
      const allowedRegions = ['kemanggisan', 'alam sutera', 'bandung', 'malang'];
      return list.filter(r => allowedRegions.includes(r.name.toLowerCase()));
    }
    return list;
  };

  return {
    regionQuery: { ...regionQuery, data: extractData(regionQuery.data, MOCK_REGIONS, true) },
